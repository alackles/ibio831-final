...
```{r models}
null.model <- glm(cancer.prop ~ 1, data=cancer.table,binomial(link="logit"))
mass.model <- glm(cancer.prop ~ mass, data=cancer.table,binomial(link="logit"))
no.interaction.model <- glm(cancer.prop  ~ genA + genB + mass, data=cancer.table,binomial(link="logit"))
interaction.model <- glm(cancer.prop  ~ genA*genB + mass, data=cancer.table,binomial(link="logit"))
summary(null.model)
summary(mass.model)
...
### AIC ###
AICtab = aictab(null.model, mass.model, no.interaction.model, interaction.model, base=TRUE, delta=TRUE, weights=TRUE, logLik=TRUE)
...
p_mass <- inv.logit(mass.model$coefficients[["(Intercept)"]] + 
            mass.model$coefficients[["mass"]] * simdata.table$cancer.prop)                           # mass predictions
p_add <- inv.logit(no.interaction.model$coefficients[["(Intercept)"]] + 
            ...
            no.interaction.model$coefficients[["mass"]] * simdata.table$cancer.prop)                 # additive predictions
...
#            interaction.model$coefficients[["mass"]] * simdata.table$cancer.prop + 
...
{plot(cancer.prop ~ mass, data=filter(cancer.table, genotype=='WT'),ylim=c(-1,1))
lines(p_null ~ mass, data=sim, lwd=2, col="cyan")
lines(p_mass ~ mass, data=sim, lwd=2, col="blue")
...
{plot(cancer.prop ~ mass, data=filter(cancer.table, genotype=='A'),ylim=c(-1,1))
lines(p_null ~ mass, data=sim, lwd=2, col="cyan")
lines(p_mass ~ mass, data=sim, lwd=2, col="blue")
lines(p_add ~ mass, data=sim, lwd=2, col="black")
lines(pAVG ~ mass, data=sim, lwd=2, col='red')}
...
{plot(cancer.prop ~ mass, data=filter(cancer.table, genotype=='B'),ylim=c(-1,1))
lines(p_null ~ mass, data=sim, lwd=2, col="cyan")
lines(p_mass ~ mass, data=sim, lwd=2, col="blue")
lines(p_add ~ mass, data=sim, lwd=2, col="black")
lines(pAVG ~ mass, data=sim, lwd=2, col='red')}
...
{plot(cancer.prop ~ mass, data=filter(cancer.table, genotype=='AB'),ylim=c(-1,1))
lines(p_null ~ mass, data=sim, lwd=2, col="cyan")
lines(p_mass ~ mass, data=sim, lwd=2, col="blue")
lines(p_add ~ mass, data=sim, lwd=2, col="black")
lines(pAVG ~ mass, data=sim, lwd=2, col='red')}
...
```{r}
# No need to scale since we only have one continuous variable and thus can't check how 1 standard deviation change in each predictor influences response
#cancer.table$mass.scaled <- scale(cancer.table$mass)
#mass.model.scaled <- glm(cancer.prop ~ mass.scaled, data=cancer.table,binomial(link="logit"))
#no.interaction.model.scaled <- glm(cancer.prop  ~ genA + genB + mass.scaled, data=cancer.table,binomial(link="logit"))
#interaction.model.scaled <- glm(cancer.prop  ~ genA*genB + mass.scaled, data=cancer.table,binomial(link="logit"))
#summary(mass.model.scaled)


control <- inv.logit(mass.model$coefficients[['(Intercept)']])
account_for_mass <- inv.logit(mass.model$coefficients[['mass']])
effect_of_accounting_for_mass <- account_for_mass - control
effect_of_accounting_for_mass         # 0.1018492

# The effect for every increased pound in a cat's mass is 10.18% increase in lifetime cancer risk.  
```
---
