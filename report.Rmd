...
plt <- ggplot(data=cancer.table, aes(x=mass.centered, y=cancer.prop, color=genotype)) + geom_jitter() + ylim(0,1)

coef.mass <- inv.logit(coef(null.model))
null.plt <- plt + geom_abline(intercept = coef.mass[[1]], slope = coef.mass[[2]])
null.plt
```
...
simdata.table$mass.centered <- scale(simdata.table$mass)
...
p_mass <- inv.logit(mass.model$coefficients[["(Intercept)"]] + 
            mass.model$coefficients[["mass.centered"]] * simdata.table$cancer.prop)                           # mass predictions
...
no.interaction.model.avgmass <- glm(cancer.prop  ~ genA + genB + mass.centered, data=cancer.table.avgmass,binomial(link="logit"))
...
#mass.centered   0.2461     3.4604   0.071    0.943
...
## interaction.model ##
interaction.model.avgmass<- glm(cancer.prop  ~ genA*genB + mass.centered, data=cancer.table.avgmass,binomial(link="logit"))
...
#mass.centered   0.2467     3.4644   0.071    0.943
