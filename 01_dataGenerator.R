...
#### Data Simulation ####
# Specify a factor variable to indicate genotype
type.names <- c("WT", "A", "B", "AB")
type <- factor(rep(type.names, each=n.reps), levels=type.names) # levels here ensures WT is the reference var
...
beta.vec.names <- c("WT", "A", "B", "AB", "WT:mass", "A:mass", "B:mass", "AB:mass")
beta.vec <- c(-0.4, 0.1, 0.2, 0.2, 0.6, 0.2, -0.5, 0.9) # we can freely edit these
